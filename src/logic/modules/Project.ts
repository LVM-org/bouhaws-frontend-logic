...
export default class Project extends Common {
  ...
  public UpdateProjectEntry = async () => {
    try {
      ...
      if (this.UpdateProjectEntryPayload.images && imagesToUpload.length > 0) {
        // upload all the images

        const apis = await Promise.all(
          imagesToUpload.map((image: any) => $api.upload.UploadImage({ image }))
        );
        const { title, description, project_entry_uuid } =
